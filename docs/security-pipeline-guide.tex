...
\begin{itemize}
  ...
  \item login rejects a missing CSRF token; and
...
npm test
\end{lstlisting}
The expected result is \texttt{tests 6}, \texttt{pass 6}, \texttt{fail 0}.

\section{Required one-time Railway and GitHub setup}

\begin{enumerate}
  \item In Railway, set real \texttt{WEB\_ADMIN\_USER}, \texttt{WEB\_ADMIN\_PASSWORD}, \texttt{TURNSTILE\_SITE\_KEY}, and \texttt{TURNSTILE\_SECRET\_KEY} variables. Do not commit them. If \texttt{auth.json} already exists on a persistent volume, plan credential rotation separately.
